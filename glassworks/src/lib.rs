...
impl Bitstream {
    pub fn new<R: Read>(mut input: R) -> io::Result<Self> {
        ...
        for row_index in 0..device.rows() {
                ...
                for bit in 0..8 {
                    if (row[column_index] & (1 << bit)) != 0 {
                        match (row_index, column_index, bit) {
                            (138, 100, 0) => println!(
                                "{row_index}:{column_index}:{bit} = peripheral bus thingy uninverted?"
                            ),
                            (138, 100, 1) => println!(
                                "{row_index}:{column_index}:{bit} = peripheral bus thingy inverted?"
                            ),
            ...
            assert_eq!(
                ecb_calc(&row[0..device.bytes_per_row() - 1]),
                row[device.bytes_per_row() - 1],
                "ECB checksum mismatch for row {row_index}"
            );
        }

        Ok(Self { device, data_type })
    }
}

#[cfg(test)]
mod tests {
    use crate::{Bitstream, ecb_calc};
    ...
    fn ecb_calc_is_correct() {
        ...
        let test = [
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0x02, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x01, 0x00, 0x00, 0x10, 0x00, 0x00,
